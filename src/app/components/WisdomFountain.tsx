...
const WisdomFountain = () => {
  ...
  const generateTopics = async () => {
    ...
    try {
      ...
      情報の中で必ず各3つ程度重要なキーワードや専門用語や大事なポイントや数字には<keyword>タグを付けてください。例: <keyword>重要な用語</keyword>
  ...
  const generateImpressionPhrases = async (
    ...
    try {
      ...
      年下が目上の人に口語調で話しているようにしてください。
      「実は」「案外」「データによると」「注目すべきなのは」「マクロ的に見ると」「本質的には」など知的に聞こえるワードを文脈に合わせて適切に含めてください。
